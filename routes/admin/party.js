...
router.post("/add", function(req,res){
    app.conn.query("select * from party_info_test where party_contact='"+req.body.party_contact+"'", function(err,result1){
        if(err){
            ...
            app.conn.query("insert into party_info_test(party_name, party_contact) values('"+req.body.party_name+"','"+req.body.party_contact+"')", function(err,result){
                if(err){
                    res.status(200).json({ status: "error", errorMessage:err.message});
                } else {
